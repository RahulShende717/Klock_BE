...
export const sipAlertDebitFailuerUpfront = async (
  trDate,
  fundcode,
  snsstatus = false
) => {
    ...
    if (!snsstatus && isDataPresent != null) {
      ...
      const calculate_result = async (array) => {
        array.forEach((item) => {
          ...
          switch (item.name) {
            case "Upfront_rejection_reasons_transactions":
              UpfrontRejectionReasonsTransactionsCalc(rows);
              break;

            case "DebitFaiure":
              DebitFaiure_calculation(rows);
              break;
        ...
        return Promise.all(
          queries.map(async (queryObj) => {
            try {
              ...
              console.log("Error in query:", error, queryObj.name);
      ...
      const main = async () => {
        ...
        let redemptionQueries = await SIPQueries.findOne(
          { endpoint: "getAlertSipAndDebitFailure" },
          { queriesArray: 1 }
        );
          ...
          { fund: fund, date: transaction_date, type: "sip" },
          {
            $set: {
              ...
              "dataObject.UpfrontRejectionReasons":
                dataObject.UpfrontRejectionReasons,
              // "dataObject.UpfrontRejectionReasonsRegistration":
              //   dataObject.UpfrontRejectionReasonsRegistration,
            },
          },
          {
