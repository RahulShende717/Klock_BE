...
const SIPTransactionData = async (trdate, fundcode, snsstatus = false) => {
  try {
    let dataObject = {};
    ...
    const isDataPresent = await CachedData.findOne({
      $and: [
        { date: transaction_date, type: "sip", fund: fund },
        {
        ...
        return Promise.all(
          queries.map(async (queryObj) => {
            try {
              ...
              console.log("Query name: ", queryObj.name);
            }
          })
        );
      };
    ...
    console.log("Error in siptransaction provider ", e);
    throw new Error(e);
  }
};

export { SIPTransactionData };
