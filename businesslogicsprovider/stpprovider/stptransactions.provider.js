...
const stpTransactionData = async (trdate, fundcode, snsstatus = false) => {
  try {
    let dataObject = {};
    ...
    const isDataPresent = await CachedData.findOne({
      $and: [
        { date: transaction_date, type: "stp", fund: fund },
        {
        ...
        array.forEach((item) => {
          ...
          switch (item.name) {
            ...
            case "Top_Performing_Scheme":
              ...
              break;
            case "Top_Performing_Scheme_Amount":
    ...
    console.log("Error in stp transaction history provider", e);
    throw new Error(e);
  }
};

export { stpTransactionData };
