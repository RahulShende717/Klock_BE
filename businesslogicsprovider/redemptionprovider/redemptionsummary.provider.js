...
export const redemptionSummary = async (
  trDate,
  fundcode,
  snsstatus = false
) => {
  try {
    ...
    if (!snsstatus && isDataPresent != null) {
      ...
      const redemption_summary_calculation = (rows, summarytype) => {
        let obj = {
          completed: {
            liquid: {
              beyond_TAT: {
      ...
      const calculate_result = async (array) => {
        array.forEach((item) => {
          ...
          switch (item.name) {
            case "redemption_disbursement":
              redemption_summary_calculation(rows, "disbursement");
              break;
            case "redemption_funding_summary":
              redemption_summary_calculation(rows, "fundingsummary");
              break;
            case "redemption_qc":
              redemption_summary_calculation(rows, "qualitycheck");
              break;
          }
        });
      };
        ...
        const cached_data = await CachedData.findOneAndUpdate(
          { fund: fund, date: transaction_date, type: "redemption" },
          {
            $set: {
              ...
              "dataObject.disbursement_summary": dataObject.disbursement,
              "dataObject.funding_summary": dataObject.fundingsummary,
    ...
    console.log("Error in redemption summary,", e);
    throw new Error(e);
  }
};
