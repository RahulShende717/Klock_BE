...
import QueryLogs from "../../models/querylogs.model.js";
import moment from "moment";

const PurchaseTransactionData = async (trdate, fundcode, snsstatus = false) => {
  try {
    let dataObject = {};
    ...
    const isDataPresent = await CachedData.findOne({
      $and: [
        { date: transaction_date, type: "purchase", fund: fund },
        {
    ...
    if (!snsstatus && isDataPresent != null) {
      ...
      const TAT_Pending_Count_Calculation = (rows) => {
        ...
        rows.forEach((ele, ind) => {
          if (ele.trdt <= transaction_date) {
            ...
            tillDateObject[assetType]["total_pending_amount"] += Number(
              ele.amount
            );
          }
          if (ele.trdt == transaction_date) {
            ...
            specificDateObject[assetType]["total_pending_amount"] += Number(
              ele.amount
            );
          }
        });
        ...
        tillDateObject["combine"]["total_pending_amount"] =
          tillDateObject["liquid"]["total_pending_amount"] +
          tillDateObject["nonliquid"]["total_pending_amount"];
        specificDateObject["combine"]["total_pending_amount"] =
          specificDateObject["liquid"]["total_pending_amount"] +
          specificDateObject["nonliquid"]["total_pending_amount"];
      ...
      // const logLongRunningQuery = async (name) => {
      //   const logEntry = {
      //     name: name,
      //     // responsetime: `Quer Timeout`,
      //     error: "Query timed out",
      //   };
      //   await QueryLogs.findOneAndUpdate(
      //     {
      ...
      //     {
      //       $set: {
      //         endpoint: "PurchaseTransactionData",
      //         $push: { queriesResponse: logEntry },
      //       },
      //     }
      //   );
      // };
      const fetch_results = async (queries) => {
        let startTime, endTime, timeTaken, obj, isQueryCompleted, timer;
        return Promise.all(
          queries.map(async (queryObj) => {
            try {
              // timer = setTimeout(async () => {

              //   await logLongRunningQuery(queryObj.name);
              // }, 300000); // 5 minutes

              // startTime = Date.now();
              // endTime = Date.now();
              // timeTaken = endTime - startTime;
              // timeTaken = timeTaken / (60 * 1000);
              // isQueryCompleted = false;
              // const timeout = setTimeout(async () => {
              //   if (!isQueryCompleted) {
              //     timeTaken = (Date.now() - startTime) / (60 * 1000); // Convert duration to minutes
              //     const logEntry = {
              //       name: queryObj.name,
              //       responsetime: `${timeTaken.toFixed(2)} minutes`,
              //       error: "Query timed out",
              //     };
              //     await QueryLogs.findOneAndUpdate(
              //       {
              ...
              //   obj = {
              //     name: queryObj.name,
              //     error: "Taking more than 5minutes",
              //     resposnetime: timeTaken,
              //   };
              //   QueryLogs.findOneAndUpdate(
              //     {
              //       date: transaction_date,
              //       fund: fund,
              //       type: "purchase",
              ...
              //   name: queryObj.name,
              //   error: e,
              //   resposnetime: `${timeTaken.toFixed(2)} minutes`,
              // };
              // await QueryLogs.findOneAndUpdate(
              //   {
              //     date: transaction_date,
    ...
    console.log("Error in purchase transaction provider purchase", e);
    throw new Error(e);
  }
};

export { PurchaseTransactionData };
