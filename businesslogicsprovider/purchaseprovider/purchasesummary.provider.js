...
const purchaseSummearyData = async (trdate, fundcode, snsstatus = false) => {
  try {
    let dataObject = {};
    ...
    if (!snsstatus && isDataPresent != null) {
      ...
      const purchase_creditMarked_Calculation = async (rows) => {
        ...
        rows.forEach((ele, ind) => {
          if (ele.asset_class == "NON LIQUID") {
            if (ele.status == "completed") {
              obj.creditmark.nonLiquid.count += Number(ele.sum);
              obj.creditmark.nonLiquid.amount += Number(ele.amount);
            ...
            if (ele.status == "completed") {
              obj.creditmark.liquid.count += Number(ele.sum);
              obj.creditmark.liquid.amount += Number(ele.amount);
              if (ele.tat == "within TAT") {
        ...
        obj.creditmark.combine.count =
          obj.creditmark.liquid.count + obj.creditmark.nonLiquid.count;
        obj.creditmark.combine.amount =
          obj.creditmark.liquid.amount + obj.creditmark.nonLiquid.amount;
      ...
      const purchase_QC_Calculation = async (rows) => {
        ...
        rows.forEach((ele, ind) => {
          if (ele.asset_class == "NON LIQUID") {
            if (ele.status == "completed") {
              obj.qualityCheck.nonLiquid.count += Number(ele.sum);
              obj.qualityCheck.nonLiquid.amount += Number(ele.amount);
              if (ele.tat == "within TAT") {
            ...
            if (ele.status == "completed") {
              obj.qualityCheck.liquid.count += Number(ele.sum);
              obj.qualityCheck.liquid.amount += Number(ele.amount);
              if (ele.tat == "within TAT") {
        ...
        obj.qualityCheck.combine.count =
          obj.qualityCheck.liquid.count + obj.qualityCheck.nonLiquid.count;
        obj.qualityCheck.combine.amount =
          obj.qualityCheck.liquid.amount + obj.qualityCheck.nonLiquid.amount;
      ...
      const purchase_Endorsement_Calculation = async (rows) => {
        ...
        rows.forEach((ele, ind) => {
          if (ele.asset_class == "NON LIQUID") {
            if (ele.status == "completed") {
              obj.endorsed.nonLiquid.count += Number(ele.count);
              obj.endorsed.nonLiquid.amount += Number(ele.amount);
              if (ele.tat == "within TAT") {
            ...
            if (ele.status == "completed") {
              obj.endorsed.liquid.count += Number(ele.count);
              obj.endorsed.liquid.amount += Number(ele.amount);
              if (ele.tat == "within TAT") {
        ...
        obj.endorsed.combine.count = obj.endorsed.liquid.count + obj.endorsed.nonLiquid.count;
        obj.endorsed.combine.amount = obj.endorsed.liquid.amount + obj.endorsed.nonLiquid.amount;
      ...
      const main = async () => {
        ...
        dataObject.radialchart = obj; // to add radial chart data which is comman for all function
    ...
    console.log("Error in purchase summary in purchase", e);
    throw new Error(e);
  }
};

export { purchaseSummearyData };
