...
export const sipRegstrationJourney = async (
  ...
  try {
    ...
    if (!snsstatus && isDataPresent != null) {
      ...
      return {
        reported_registration: isDataPresent.dataObject.reported_registration,
        pending_registration: isDataPresent.dataObject.pending_registration,
        successfully_registered:
          isDataPresent.dataObject.successfully_registered,
        rejected_registration: isDataPresent.dataObject.rejected_registration,
        sent_to_aggregator: isDataPresent.dataObject.sent_to_aggregator,
      };
    } else {
      const Reported_registration_calculation = (rows) => {
        ...
        rows.forEach((ele, ind) => {
          ...
          obj.reported_registration_combine.count += Number(ele.txncount);
          if (ele.asset_class == "LIQUID") {
            obj.reported_registration_liquid.amount += Number(ele.amt);
            obj.reported_registration_liquid.count += Number(ele.txncount);
          }
          if (ele.asset_class == "NON LIQUID") {
            obj.reported_registration_nonliquid.amount += Number(ele.amt);
            obj.reported_registration_nonliquid.count += Number(ele.txncount);
          }
        });
      ...
      const successfully_registered_calculation = (rows) => {
        ...
        rows.forEach((ele, ind) => {
          ...
          obj.successfully_registered_combine.count += Number(ele.txncount);
          if (ele.asset_class == "LIQUID") {
            obj.successfully_registered_liquid.amount += Number(ele.amt);
            obj.successfully_registered_liquid.count += Number(ele.txncount);
          }
          if (ele.asset_class == "NON LIQUID") {
            obj.successfully_registered_nonliquid.amount += Number(ele.amt);
            obj.successfully_registered_nonliquid.count += Number(ele.txncount);
          }
        });
      ...
      const pending_registration_calculation = (rows) => {
        ...
        rows.forEach((ele, ind) => {
          obj.pending_registration_combine.amount += Number(ele.amount);
          obj.pending_registration_combine.count += Number(ele.trxns);
          if (ele.asset_class == "LIQUID") {
            obj.pending_registration_liquid.amount += Number(ele.amount);
            obj.pending_registration_liquid.count += Number(ele.trxns);
          }
          if (ele.asset_class == "NON LIQUID") {
            obj.pending_registration_nonliquid.amount += Number(ele.amount);
            obj.pending_registration_nonliquid.count += Number(ele.trxns);
          }
        });
      ...
      const rejected_registration_calculation = (rows) => {
        ...
        rows.forEach((ele, ind) => {
          obj.rejected_registration_combine.amount += Number(ele.amount);
          obj.rejected_registration_combine.count += Number(ele.trxns);
          if (ele.asset_class == "LIQUID") {
            obj.rejected_registration_liquid.amount += Number(ele.amount);
            obj.rejected_registration_liquid.count += Number(ele.trxns);
          }
          if (ele.asset_class == "NON LIQUID") {
            obj.rejected_registration_nonliquid.amount += Number(ele.amount);
            obj.rejected_registration_nonliquid.count += Number(ele.trxns);
          }
        });
      ...
      const main = async () => {
        ...
        //           query: `Select distinct asset_class,count(distinct trxns) trxns,sum(amount) as amount from
        // (Select distinct asset_class,concat(
        // 				sip_fund,
        // 				sip_scheme,
        // 				sip_plan,
        // 				sip_acno,
        //                 sip_branch,
        // 				sip_ihno,
        // 				sip_regslno) as trxns ,cast(nullif(sip_indamt,'') as decimal) as amount
        // 	from lic.sipregistrationfinal where substring(sip_regdt,1,10) ='${transaction_date}' and dispatchstatus='Y'
        // 	and exists (Select distinct * from lic.newmandateregistrationdetails_mirror nm where nm.parentihno=sip_ihno
        // and nm.folionumber=sip_acno)
        // 	UNION
        // 	Select distinct asset_class,concat(
        // 				sip_fund,
        // 				sip_scheme,
        ...
        // 				sip_ihno,
        // 				sip_regslno) as trxns ,cast(nullif(sip_indamt,'') as decimal) as amount
        // 	from lic.sipregistrationnonfinal where substring(sip_regdt,1,10) ='${transaction_date}' and dispatchstatus='Y'
        // 		and exists (Select distinct * from lic.newmandateregistrationdetails_mirror nm where nm.parentihno=sip_ihno
        // and nm.folionumber=sip_acno)
        // 	)x
        // 		group by 1
